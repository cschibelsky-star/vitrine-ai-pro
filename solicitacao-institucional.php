...
?>
<section class="section"><span class="eyebrow dark">Solicitação comercial</span><h1>Solicite apresentação, escopo ou proposta.</h1><p class="lead">Este formulário registra sua oportunidade comercial e encaminha as informações para análise. Para órgãos públicos, a contratação continua sujeita aos procedimentos administrativos e legais aplicáveis.</p>
<form class="formGrid" method="post" action="/salvar-solicitacao.php"><?= csrf_field() ?>
<label>Tipo de organização<select name="perfil" required><option value="">Selecione</option><?php foreach(['Prefeitura ou Secretaria','Câmara Municipal','Autarquia / Fundação / Consórcio','Portal de Notícias / Rádio / Jornal','TV Web / Projeto Audiovisual','Empresa / Comércio / Prestador de Serviços','Operador de Turismo / Cidade','Marca / Profissional / Equipe de Comunicação','Instituição de Ensino / Projeto Educacional','Outro'] as $op): ?><option<?= $prefPerfil===$op?' selected':'' ?>><?= e($op) ?></option><?php endforeach; ?></select></label>
...
<label class="full">Necessidade principal<textarea name="necessidade" required placeholder="Descreva o que você precisa."><?= e($prefNecessidade) ?></textarea></label>
