...
class CommercialFactoryIntakeService
{
    ...
    public function intake(array $data, bool $dryRun = true): array
    {
        ...
        File::put($base . '/commercial_intake.json', json_encode(['client'=>$data['client'], 'product'=>$resolved['name'], 'plan'=>$planKey, 'project_slug'=>$projectSlug, 'prompt'=>$prompt], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
        $exitCode = Artisan::call('factory:build-and-install', ['request' => [$prompt], '--dry-run' => true]);
        $report = [
            'status' => $exitCode === 0 ? 'finished' : 'failed',
            'project_slug' => $projectSlug,
            'commercial_status' => $exitCode === 0 ? 'ready_for_homologation' : 'factory_failed',
